...
class Database:
    ...
    @staticmethod
    def _build_url_to_connect(username: str, password: str, database: str, host: str = 'localhost') -> str:
        return f'postgresql+asyncpg://{username}:{password}@{host}/{database}'

    async def connect(self, *args, **kwargs) -> None:
        self._db = db
        url_to_connect: str = self._build_url_to_connect(username=self.app.config.database.user,
                                                         password=self.app.config.database.password,
                                                         database=self.app.config.database.database)
        self._engine = create_async_engine(url_to_connect,
                                           echo=True,
                                           future=True)
